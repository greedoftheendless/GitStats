...
export default function ChatScreen({
    ...
    useEffect(() => {
        ...
        socket.on('chat-cleared', ({ type }) => {
            setMessages([])
            setFiles([])
            addSystemMessage(type === 'both' ? 'Chat was cleared by consensus' : 'You cleared the chat locally')
            ...
            <header className="bg-slate-800 border-b border-slate-700 px-6 py-4 flex items-center justify-between z-20">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        <h1 className="text-lg font-bold text-white truncate font-mono tracking-widest">{sessionId}</h1>
